...
|---|---|---|---|---|---|---|
| 4 | `0xe7cf14dab5b786aa9fc4d23b13fca34eefb771ace69097d274aacd027e668deb` | `a0-6jxLQeD_IsZ8wcXTH5vELWkrcJYKU39b2LPNuBPg` | 1.27 MiB | 34 | ~2026-07-06 | **Token icons + UX polish** — Whitelist tokens (SUI, USDC, D) now ship branded SVG icons under `/tokens/*.svg` with letter-monogram fallback for custom coins. New `<TokenIcon>` component renders 14–20px circles in the trade dropdowns (selected-token icon left of `<select>`, since native HTML strips images from `<option>`), pool list rows (overlapping pair icons), Create Pool form, balance chips, and amount-suffix labels. D = orange "D" on black (matching ONE Aptos sibling), USDC = Circle blue $-on-blue (verbatim from sibling), SUI = blue droplet glyph. Token Factory wizard now shows a live "Auto-generated identifiers" preview box under the symbol input (display symbol, OTW struct/module, package alias, source filename) so creators understand the wizard handles Sui's OTW uppercase rule automatically. Donate page gained an irrevocability warning (D→SP economically equivalent to burn via liquidation absorption; SUI→reserve permanently locked but flows out via redeem_from_reserve, not burned). |
